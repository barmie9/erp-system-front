...
const AddOrder = ({ refreshParent }) => {
    ...
    const handleAddOrder = async () => {
        ...
        if (checkFields([orderName, quantity, datePicker, userId, companyId])) {
            const endDate = formatDateToStr(datePicker);
            const response = await ApiDataService.addOrder(orderName, quantity, formatDateToStr(today), endDate, endDate, userId, companyId);
            setOrderName('');
            setQuantity('');
            setDatePicker(null);
            setCompanyStr('');
